    ...
    def __init__(self, path: Optional[Path] = None) -> None:
        ...
        if path:
            ...
            try:
                ...
                logging.error(
                    f"The following error occurred while retrieving the dataset from '{path}' 🛑\n---\n{e}\n---"
                )
...
def type_annotation_process(q_in, dataset: CombinatorsDataset) -> None:
    while True:
        ...
        if isinstance(batch, list):
            ...
            results = [(annotate(tree)["type"], expr) for (expr, tree) in parsed_pairs]
            if results:
...
if __name__ == "__main__":
    ...
    try:
        if not DATASET_PATH.exists():
            ...
            DATASET_PATH.touch()
        dataset = manager.CombinatorsDataset(DATASET_PATH)  # type: ignore
    except Error:
    ...
    try:
        ...
        logging.info(f"Dumping test set to {DATASET_PATH / 'train.jsonl'}...")
        CombinatorsDataset.to_jsonl(DATASET_PATH / "train.jsonl", train)

        logging.info(
            f"Dumping validation set to {DATASET_PATH / 'validation.jsonl'}..."
        )
        CombinatorsDataset.to_jsonl(DATASET_PATH / "validation.jsonl", val)

        logging.info(f"Dumping test set to {DATASET_PATH / 'test.jsonl'}...")
